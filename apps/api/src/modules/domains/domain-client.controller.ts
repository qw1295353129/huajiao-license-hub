...
import { ApiKeyGuard, type ApiKeyContext } from '../../common/guards/api-key.guard';
import { ApiKeyCtx, ClientIp, Public, RequireScopes } from '../../common/decorators';
...
export class DomainClientController {
  ...
  activate(
    @Body() dto: DomainActivateDto,
    @ClientIp() ip: string,
    @ApiKeyCtx() apiKey?: ApiKeyContext,
  ) {
    return this.domains.activate(dto, { ip, apiKeyProductId: apiKey?.productId ?? null });
  }

  @Post('verify')
  @RequireScopes('license:verify')
  @ApiOperation({ summary: '域名心跳校验' })
  verify(@Body() dto: DomainVerifyDto, @ApiKeyCtx() apiKey?: ApiKeyContext) {
    return this.domains.verify(dto, { apiKeyProductId: apiKey?.productId ?? null });
  }

  @Post('deactivate')
  @RequireScopes('license:deactivate')
  @ApiOperation({ summary: '域名自助解绑（释放额度）' })
  deactivate(@Body() dto: DomainDeactivateDto, @ApiKeyCtx() apiKey?: ApiKeyContext) {
    return this.domains.deactivate(dto, { apiKeyProductId: apiKey?.productId ?? null });
  }
